...
def modificar():
    ...
    while not salir: 
        ...
        try:
            modificar_opcion = int(input("Ingrese una opción: "))
            ...
            elif modificar_opcion == 2:
                print('PRODUCTOS FRACCIONADOS DISPONIBLES: ')
                mostrar_productos_fraccionados_disponibles()
                print('')
                modificar_presentacion(cursor, conn)
                print("\nPRODUCTO FRACCIONADO fue MODIFICADO con éxito.\n")
            elif modificar_opcion == 3:
                mostrar_proveedores()
                modificar_proveedor(cursor, conn)
                print("\nEl PROVEEDOR fue MODIFICADO con éxito.\n")
            elif modificar_opcion == 4:
...
def modificar_producto(cursor, conn):
    try:
        ...
        query = """
            UPDATE productosgranel 
            SET descripcion = %s, cantidad = %s, tamano = %s, 
                unidadmedida = %s, cantidadmax = %s, cantidadmin = %s, 
                idcategoria = %s 
            WHERE idproductogranel = %s
        """
        values = (descripcion, cantidad, tamano, unidad_medida, cantidad_max, cantidad_min, id_categoria, id)
        cursor.execute(query, values)
        conn.commit()
        print("Producto a granel modificado con éxito.")

    except Exception as e:
        print(f"Error al modificar el producto: {e}")

def modificar_presentacion(cursor, conn):
    try:
        ...
        query = """
            ...
            SET descripcion = %s, cantidad = %s, tamano = %s, 
                unidadmedida = %s, cantidadmax = %s, cantidadmin = %s 
            WHERE idproductofracc = %s
        """
        values = (descripcion, cantidad, tamano, unidad_medida, cantidad_max, cantidad_min, id)
        cursor.execute(query, values)
        conn.commit()
        print("Producto fraccionado modificado con éxito.")

    except Exception as e:
        print(f"Error al modificar el producto fraccionado: {e}")

def modificar_proveedor(cursor, conn):
    try:
        ...
        query = """
            UPDATE proveedores 
            SET CUIT = %s, proveedor = %s, direccion = %s,  
                telefono = %s, email = %s, tiempodeentrega = %s 
            WHERE CUIT = %s
        """
        ...
        print(f"Error al modificar el proveedor: {e}")